...
The project includes a full **Storybook component library** documenting 15 components across three tiers, with interactive controls, docs, and interaction tests.
...
| Component | Variants | Interaction Tests |
|-----------|----------|-------------------|
| `Button` | primary, secondary, ghost, danger, outline, link, sizes, loading, disabled, icons | Click, loading-prevents-click |
| `Chip` / StatusChip | default, success, warning, failed, info + domain statuses (plan, invoice, payment, subscription) | Clickable chip |
| `Input` | text, number, formatted-number, integer, prefix, suffix, error, sizes | Typing interaction |
| `Select` | standard, radio mode, descriptions, disabled | - |
| `Tooltip` | all placements, delays, rich content | - |
| `Spinner` | sizes, colors, inline + centered | - |

**Molecules** (6 components)

| Component | Key Features |
|-----------|-------------|
| `MetricCard` | KPI cards with trend indicators, currency formatting, dashboard grid |
| `DataTable` | Sortable columns, loading skeletons, pagination, **virtualisation (10K/50K rows)** |
| `InvoiceStatusBadge` | Maps status strings to coloured chips with icons |
| `UsageBar` | Progress bar with auto warning/danger threshold colors |
| `DateRangePicker` | Calendar-based range selection with presets |
...
| Component | Key Features |
|-----------|-------------|
| `SidebarNav` | Collapsible, nested accordion, active route highlighting, header/footer slots |
| `PricingTierTable` | Volume + slab pricing tiers, flat fees, currency formatting |
| `EmptyState` | Icon, headline, subtext, CTA button with domain-specific variants |

### Advanced Challenges

- **Challenge A - Filter Persistence**: `useFilterStore` (Zustand) with sessionStorage persistence and URL fingerprint syncing
- **Challenge B - Virtualised List**: `@tanstack/react-virtual` integration in DataTable for 10K+ row rendering
- **Challenge C - Query Config**: `createQueryConfig` utility with REALTIME/DEFAULT/STATIC presets for TanStack Query v5

### Tests

- **160 tests passing** across 14 test files
- 3+ utility test suites (`format_number`, `helper_functions`, `price_helpers`)
- 3+ component test suites (`Button`, `Chip`, `Input`)
- Query config test suite with 13 tests
